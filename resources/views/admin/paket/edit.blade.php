    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
                ...
                <h2 class="page-title">Edit Paket Internet</h2>
            </div>
        </div>
    </div>
</div>
                ...
                <form method="POST" action="{{ route('admin.paket.update', $paket->id) }}">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" name="speed"
                                    ...
                                    value="{{ old('speed', $paket->speed) }}" required>
                                @error('speed')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                            <option value="aktif" {{ old('status', $paket->status) == 'aktif' ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="nonaktif"
                                {{ old('status', $paket->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                        @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
